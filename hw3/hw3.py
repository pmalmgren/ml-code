...
def main():
    ...
    x_test = vectorData[:,0:fourth]
    y_test = labels[0:fourth]
    x_validate = vectorData[:,fourth:fourth*2]
    y_validate = labels[fourth:fourth*2]
    x_train = vectorData[:,fourth*2:-1]
    y_train = labels[fourth*2:]
    
    # train a model with a stump, perceptron, or both.
    model = boost.train(x_train,y_train,x_validate,y_validate,learner='perceptron')
